...
with tf.name_scope("hidden_layer_1"):
	W1 = tf.Variable(tf.random_normal(shape=(7,n)) * tf.sqrt(2/n), name="W1", dtype=tf.float32)
	b1 = tf.Variable(tf.zeros(shape=(7,1)), name="b1", dtype=tf.float32)
...
with tf.name_scope("hidden_layer_2"):
	W2 = tf.Variable(tf.random_normal(shape=(7,7)) * tf.sqrt(2/7),name="W2", dtype=tf.float32)
	b2 = tf.Variable(tf.zeros(shape=(7,1)), name="b2", dtype=tf.float32)
...
with tf.name_scope("output_layer"):
	W3 = tf.Variable(tf.random_normal(shape=(1,7)) * tf.sqrt(2/7),name="W3", dtype=tf.float32)
	b3 = tf.Variable(tf.zeros(shape=(1,1)), name="b3", dtype=tf.float32)
...
with tf.Session() as sess:
	...
	for i in range(iterations):
		...
		step, cost, summary = sess.run([train_step,cross_entropy,merged],feed_dict={x:train_x_orig, y_:train_y})
